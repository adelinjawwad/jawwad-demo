...
function loadAllAssets() {
  ...
  setTimeout(async () => {
    let fullContent = `
      <aside class="fixed top-24 left-4 space-y-2 z-50">
        ${categories.map(cat => `<a href="#${cat}" class="block px-3 py-2 text-white bg-gray-800 rounded hover:bg-pink-600 transition text-sm" id="link-${cat}">${categoryTitles[cat]}</a>`).join('')}
      </aside>
      <div class="ml-32 space-y-12">
    `;
...
function renderItem(item, section) {
  ...
  if (section === 'free') {
    return `
      <div class="bg-gradient-to-br from-gray-800 to-gray-900 rounded-none shadow-lg overflow-hidden transition duration-300">
        ${imageWithPreviewButton}
        <div class="p-4">
          <h3 class="text-lg font-medium text-white">${item.title}</h3>
          ${item.download ? `
            <a href="${item.download}" download class="mt-4 block w-full py-2 bg-gradient-to-r from-green-500 to-teal-600 text-white text-center rounded-none hover:from-green-600 hover:to-teal-700 transition">
