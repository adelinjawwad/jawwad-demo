...
if (section === 'tools') {
  return `
    <div class="bg-black bg-opacity-30 rounded-none p-6 flex flex-col h-full relative">
      <h3 class="text-2xl font-bold text-white mb-4">${item.title}</h3>

      <div class="relative mb-4">
        ${item.onSale ? `
          <div class="absolute top-2 left-2 bg-gradient-to-br from-red-600 to-yellow-500 text-white text-sm px-3 py-1.5 rounded-lg font-extrabold z-10 shadow-lg animate-bounce tracking-wide uppercase">
